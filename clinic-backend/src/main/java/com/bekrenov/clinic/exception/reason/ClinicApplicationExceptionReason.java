...
public enum ClinicApplicationExceptionReason implements ExceptionReasonPolicy {
    ...
    CANNOT_DELETE_DEPARTMENT_WITH_EMPLOYEES("Department with id [%s] still has associated employees and therefore cannot be deleted", HttpStatus.CONFLICT),
    ...
    CANNOT_FINISH_APPOINTMENT("Only appointments with status CONFIRMED can be finished", HttpStatus.BAD_REQUEST);
